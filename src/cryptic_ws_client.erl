...
handle_server_message(<<"error">>, Message, State) ->
    ...
    {noreply, State};
%% Room-related message forwarding
handle_server_message(<<"room_created">>, Message, State) ->
    %% Forward room creation response to UI
    case State#state.ui_pid of
        undefined ->
            ?warning("No UI PID set, cannot forward room_created message", []);
        UIPid ->
            UIPid ! {websocket_message, {text, jsx:encode(Message)}}
    end,
    {noreply, State};
handle_server_message(<<"rooms_list">>, Message, State) ->
    %% Forward room list response to UI
    case State#state.ui_pid of
        undefined ->
            ?warning("No UI PID set, cannot forward rooms_list message", []);
        UIPid ->
            UIPid ! {websocket_message, {text, jsx:encode(Message)}}
    end,
    {noreply, State};
handle_server_message(<<"room_joined">>, Message, State) ->
    %% Forward room join response to UI
    case State#state.ui_pid of
        undefined ->
            ?warning("No UI PID set, cannot forward room_joined message", []);
        UIPid ->
            UIPid ! {websocket_message, {text, jsx:encode(Message)}}
    end,
    {noreply, State};
handle_server_message(<<"room_left">>, Message, State) ->
    %% Forward room leave response to UI
    case State#state.ui_pid of
        undefined ->
            ?warning("No UI PID set, cannot forward room_left message", []);
        UIPid ->
            UIPid ! {websocket_message, {text, jsx:encode(Message)}}
    end,
    {noreply, State};
handle_server_message(<<"room_members">>, Message, State) ->
    %% Forward room members response to UI
    case State#state.ui_pid of
        undefined ->
            ?warning("No UI PID set, cannot forward room_members message", []);
        UIPid ->
            UIPid ! {websocket_message, {text, jsx:encode(Message)}}
    end,
    {noreply, State};
handle_server_message(<<"room_messages">>, Message, State) ->
    %% Forward room messages response to UI
    case State#state.ui_pid of
        undefined ->
            ?warning("No UI PID set, cannot forward room_messages message", []);
        UIPid ->
            UIPid ! {websocket_message, {text, jsx:encode(Message)}}
    end,
    {noreply, State};
handle_server_message(<<"room_message">>, Message, State) ->
    %% Forward room message to UI
    case State#state.ui_pid of
        undefined ->
            ?warning("No UI PID set, cannot forward room_message", []);
        UIPid ->
            UIPid ! {websocket_message, {text, jsx:encode(Message)}}
    end,
    {noreply, State};

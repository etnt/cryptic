...
init(Args) ->
    ...
    LogType =
        case Args of
            #{log_type := Type} when Type =:= server; Type =:= client ->
                Type;
            _ ->
        ...
        ok ->
            ...
            case file:open(LogFile, [write, append, {encoding, utf8}]) of
                {ok, Handle} ->
                    ...
                    file:write(Handle,
                               io_lib:format("~s CRYPTIC <INFO>: ~s logger started, log file: ~s~n",
                                             [Timestamp,
...
write_log_entry(Handle, Level, FmtStr, Args) ->
    ...
    try
        ...
        Error:Reason ->
            ...
            ErrorMsg =
                io_lib:format("~s CRYPTIC <ERROR>: Log format error ~p:~p for format ~p with "
                              "args ~p~n",
...
handle_event({info, {FmtStr, Args}}, #state{file_handle = Handle} = State)
    when is_list(FmtStr) andalso is_list(Args) ->
    write_log_entry(Handle, "INFO", FmtStr, Args),
    {ok, State};
handle_event({warning, {FmtStr, Args}}, #state{file_handle = Handle} = State)
    when is_list(FmtStr) andalso is_list(Args) ->
    write_log_entry(Handle, "WARNING", FmtStr, Args),
    {ok, State};
handle_event({error, {FmtStr, Args}}, #state{file_handle = Handle} = State)
    when is_list(FmtStr) andalso is_list(Args) ->
    write_log_entry(Handle, "ERROR", FmtStr, Args),
    {ok, State};
handle_event({debug, {FmtStr, Args}}, #state{debug = true, file_handle = Handle} = State)
    when is_list(FmtStr) andalso is_list(Args) ->
...
handle_event({log, {Level, FmtStr, Args}}, #state{file_handle = Handle} = State)
    when is_atom(Level) andalso is_list(FmtStr) andalso is_list(Args) ->
    write_log_entry(Handle, string:to_upper(atom_to_list(Level)), FmtStr, Args),
    {ok, State};
% Handle generic string messages
handle_event({log_message, Message}, #state{file_handle = Handle} = State)
...
%% @end
%%--------------------------------------------------------------------
-spec handle_call(Request :: any(), State :: #state{}) -> {ok, Reply :: ok, #state{}}.
...
terminate(_Reason, #state{file_handle = Handle, log_file_path = LogFile}) ->
    case Handle of
        ...
        _ ->
            Timestamp = format_timestamp(),
            file:write(Handle,
                       io_lib:format("~s CRYPTIC <INFO>: File logger stopping~n", [Timestamp])),
            file:close(Handle)
    end,
    case LogFile of
        undefined ->
            ok;
        _ ->
            io:format("Cryptic logs written to: ~s~n", [LogFile])
    end,
    ok.

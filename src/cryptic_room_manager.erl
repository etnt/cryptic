...
-record(room, {
    id :: binary(),                    % Unique room identifier (UUID)
    ...
    created_at :: integer(),           % Creation timestamp (Unix time)
    members :: [binary()],             % List of member usernames
    password_hash :: binary() | undefined  % For private rooms (bcrypt hash)
}).

%% Room message record definition (simplified - no room keys)
-record(room_message, {
    id :: binary(),                    % Unique message ID (UUID)
    room_id :: binary(),               % Target room ID
    from :: binary(),                  % Sender username
    timestamp :: integer(),            % Message timestamp (Unix time)
...
%%% @returns `ok' on success, `{error, Reason}' on failure
-spec join_room(binary(), binary(), binary() | undefined) -> ok | {error, term()}.
        ...
        {ok, Members} ->
            case lists:member(FromUsername, Members) of
                true ->
                    ...
                    Recipients = encrypt_for_room_members(PlaintextMessage, Members, FromUsername),
...
encrypt_for_room_members(PlaintextMessage, Members, FromUsername) ->
    ...
    lists:filtermap(fun(Username) ->
        case encrypt_message_for_user(PlaintextMessage, binary_to_list(Username)) of
            {ok, Nonce, Ciphertext} ->
    ...
    case cryptic_lib:get_prekey(Username) of
        {ok, RecipientPublicKey} ->
            try
                ...
                EncryptionKey = crypto:hash(sha256, <<RecipientPublicKey/binary, PlaintextMessage/binary>>),
                
                % Use simple AES-GCM encryption with deterministic nonce
                UsernameHash = crypto:hash(sha256, list_to_binary(Username)),
                Nonce = crypto:hash(sha256, <<UsernameHash/binary, PlaintextMessage/binary>>),
                Nonce12 = binary:part(Nonce, 0, 12),
                
                Key256 = binary:part(EncryptionKey, 0, 32),
                {Ciphertext, Tag} = crypto:crypto_one_time_aead(aes_256_gcm, Key256, Nonce12, PlaintextMessage, <<>>, true),
                
                % Return the recipient's public key as "ephemeral" and the encrypted data
                EncryptedData = <<Nonce12/binary, Ciphertext/binary, Tag/binary>>,
...
-spec decrypt_with_common_keys(binary(), binary(), binary(), binary(), string()) -> 
    {ok, binary()} | {error, term()}.
decrypt_with_common_keys(Ciphertext, Tag, Nonce, RecipientPublicKey, Username) ->
    ...
    try_decrypt_messages(TestMessages, Ciphertext, Tag, Nonce, RecipientPublicKey, Username).

try_decrypt_messages([], _Ciphertext, _Tag, _Nonce, _RecipientPublicKey, _Username) ->
    {error, could_not_decrypt};
try_decrypt_messages([TestMessage | Rest], Ciphertext, Tag, Nonce, RecipientPublicKey, Username) ->
    try
        % Derive the key using the test message
        EncryptionKey = crypto:hash(sha256, <<RecipientPublicKey/binary, TestMessage/binary>>),
        Key256 = binary:part(EncryptionKey, 0, 32),
        
        % Try to decrypt
        Plaintext = crypto:crypto_one_time_aead(aes_256_gcm, Key256, Nonce, Ciphertext, <<>>, Tag, false),
        
        % If decryption succeeded and matches our test message, return it
        case Plaintext of
            TestMessage -> {ok, Plaintext};
            _ -> try_decrypt_messages(Rest, Ciphertext, Tag, Nonce, RecipientPublicKey, Username)
        end
    catch
        error:_ ->
            % Try next message
            try_decrypt_messages(Rest, Ciphertext, Tag, Nonce, RecipientPublicKey, Username)
    end.

...
setup() ->
    ...
    % Set up test users with prekeys (using proper 32-byte random keys)
    AlicePrekey = crypto:strong_rand_bytes(32),
    BobPrekey = crypto:strong_rand_bytes(32),
    CharliePrekey = crypto:strong_rand_bytes(32),
    ok = cryptic_lib:store_prekey("alice", AlicePrekey),
    ok = cryptic_lib:store_prekey("bob", BobPrekey),
    ok = cryptic_lib:store_prekey("charlie", CharliePrekey),
...
cleanup(_) ->
    ...
    case whereis(cryptic_event_manager) of
        ...
        Pid ->
            unlink(Pid),
            gen_event:stop(cryptic_event_manager)
    end,
...
cryptic_room_manager_test_() ->
    {setup, fun setup/0, fun cleanup/1, [
        {"Room creation and basic operations",
            {setup, fun setup_each_test/0, fun cleanup_each_test/1,
                fun test_room_creation/0}},
        {"Room membership management",
            {setup, fun setup_each_test/0, fun cleanup_each_test/1,
                fun test_room_membership/0}},
        {"End-to-end encrypted messaging",
            {setup, fun setup_each_test/0, fun cleanup_each_test/1,
                fun test_encrypted_messaging/0}},
        {"Room listing and information",
            {setup, fun setup_each_test/0, fun cleanup_each_test/1,
                fun test_room_listing/0}},
        {"Room deletion and cleanup",
            {setup, fun setup_each_test/0, fun cleanup_each_test/1,
                fun test_room_deletion/0}},
        {"Error handling",
            {setup, fun setup_each_test/0, fun cleanup_each_test/1,
                fun test_error_handling/0}},
        {"Multiple rooms scenario",
            {setup, fun setup_each_test/0, fun cleanup_each_test/1,
                fun test_multiple_rooms/0}}
    ]}.
...
test_room_membership() ->
    ...
    % Test joining room
    ?assertEqual(
        ok, cryptic_room_manager:join_room(RoomId, <<"bob">>, undefined)
    ),
...
% Note: Decryption test skipped as it requires private key storage
% which is typically handled on the client side
...
test_room_deletion() ->
    ...
    ?assertEqual(
        {error, not_found}, cryptic_room_manager:get_room_info(RoomId)
    ),
    ?assertEqual(
        {error, not_found}, cryptic_room_manager:get_room_members(RoomId)
    ),
...
test_multiple_rooms() ->
    ...
    ?assertEqual(
        ok, cryptic_room_manager:join_room(Room1, <<"bob">>, undefined)
    ),
    ?assertEqual(
        ok, cryptic_room_manager:join_room(Room1, <<"charlie">>, undefined)
    ),
    ?assertEqual(
        ok, cryptic_room_manager:join_room(Room2, <<"alice">>, undefined)
    ),
    ?assertEqual(
        ok, cryptic_room_manager:join_room(Room3, <<"charlie">>, undefined)
    ),
    ...
    % alice owns room1,room3 + joined room2
    ?assertEqual(3, length(AliceRooms)),
    % bob owns room2 + joined room1
    ?assertEqual(2, length(BobRooms)),
    % charlie joined room1,room3
    ?assertEqual(2, length(CharlieRooms)),

...
-record(ratchet_state, {
    root_key, send_chain_key, send_msg_number,
    recv_chain_key, recv_msg_number, prev_recv_chain_length,
    dh_self, dh_remote, dh_ratchet_step, skipped_keys,
    max_skip, max_cache_size, max_cache_age,
    sending_chain_active, receiving_chain_active,
...
setup_alice_bob_ratchets() ->
    ...
    % Generate DH keypairs for Alice and Bob
    {AliceDHPub, AliceDHPriv} = cryptic_nif:gen_keypair(),
    {BobDHPub, BobDHPriv} = cryptic_nif:gen_keypair(),
    
    % Alice initializes as sender
    AliceState = cryptic_double_ratchet:init_sender(RootKey, {AliceDHPub, AliceDHPriv}),
    
    % Bob initializes as receiver with Alice's DH public key
    BobState = cryptic_double_ratchet:init_receiver(RootKey, {BobDHPub, BobDHPriv}, AliceDHPub),
...
init_receiver_test() ->
    RootKey = cryptic_nif:rand_bytes(32),
    {DHPub, DHPriv} = cryptic_nif:gen_keypair(),
    {RemoteDHPub, _} = cryptic_nif:gen_keypair(),
    
    State = cryptic_double_ratchet:init_receiver(RootKey, {DHPub, DHPriv}, RemoteDHPub),
    ...
    ?assertEqual(0, State#ratchet_state.recv_msg_number),
    ?assertEqual({DHPub, DHPriv}, State#ratchet_state.dh_self),
    ?assertEqual(RemoteDHPub, State#ratchet_state.dh_remote),
    ?assertEqual(false, State#ratchet_state.sending_chain_active),
    ?assertEqual(true, State#ratchet_state.receiving_chain_active),
    ?assertEqual(#{}, State#ratchet_state.skipped_keys),
    ?assertEqual(1, State#ratchet_state.dh_ratchet_step). % Should have performed initial DH ratchet

init_invalid_params_test() ->
    % Test with invalid root key size
    ?assertError(function_clause, cryptic_double_ratchet:init_sender(<<"short">>, {<<"a">>, <<"b">>})),
    
    % Test with invalid DH key sizes
    RootKey = cryptic_nif:rand_bytes(32),
    ?assertError(function_clause, cryptic_double_ratchet:init_sender(RootKey, {<<"short">>, <<"also_short">>})).
...
key_derivation_test() ->
    ...
    {NewRootKey, SendChainKey, RecvChainKey} = cryptic_double_ratchet:kdf_rk(RootKey, DHOutput),
...
chain_key_advancement_test() ->
    ...
    {NewSendChain, SendMsgKey} = cryptic_double_ratchet:advance_sending_chain(ChainKey, MsgNumber),
    ...
    {NewRecvChain, RecvMsgKey} = cryptic_double_ratchet:advance_receiving_chain(ChainKey, MsgNumber),
    ...
    % Send and receive chains should produce different keys
    ?assertNotEqual(NewSendChain, NewRecvChain),
    ?assertNotEqual(SendMsgKey, RecvMsgKey).
...
encrypt_decrypt_basic_test() ->
    ...
    {ok, Message, AliceState2} = cryptic_double_ratchet:encrypt_message(Plaintext, AliceState),
    ...
    ?assertNotEqual(AliceState#ratchet_state.send_chain_key, AliceState2#ratchet_state.send_chain_key),
    
    % Bob decrypts the message (placeholder implementation for Phase 1)
    {ok, DecryptedText, BobState2} = cryptic_double_ratchet:decrypt_message(Message, BobState),
    
    % For Phase 1, we expect placeholder text
    ?assertEqual(<<"placeholder_plaintext">>, DecryptedText),
...
encrypt_multiple_messages_test() ->
    ...
    {FinalState, EncryptedMessages} = lists:foldl(
        fun(Plaintext, {State, AccMessages}) ->
            {ok, Message, NewState} = cryptic_double_ratchet:encrypt_message(Plaintext, State),
    ...
    MessageNumbers = [maps:get(msg_number, M) || M <- lists:reverse(EncryptedMessages)],
    ?assertEqual([0, 1, 2], MessageNumbers),
    
    % Verify final state
    ?assertEqual(3, FinalState#ratchet_state.send_msg_number),
    ?assertNotEqual(AliceState#ratchet_state.send_chain_key, FinalState#ratchet_state.send_chain_key).
...
serialize_deserialize_test() ->
    ...
    {ok, DeserializedState} = cryptic_double_ratchet:deserialize_state(SerializedData),
    
    % Verify deserialized state matches original
    ?assertEqual(AliceState#ratchet_state.root_key, DeserializedState#ratchet_state.root_key),
    ?assertEqual(AliceState#ratchet_state.send_chain_key, DeserializedState#ratchet_state.send_chain_key),
    ?assertEqual(AliceState#ratchet_state.send_msg_number, DeserializedState#ratchet_state.send_msg_number),
    ?assertEqual(AliceState#ratchet_state.dh_self, DeserializedState#ratchet_state.dh_self),
    ?assertEqual(AliceState#ratchet_state.dh_remote, DeserializedState#ratchet_state.dh_remote),
    ?assertEqual(AliceState#ratchet_state.sending_chain_active, DeserializedState#ratchet_state.sending_chain_active),
    ?assertEqual(AliceState#ratchet_state.receiving_chain_active, DeserializedState#ratchet_state.receiving_chain_active).
...
get_state_info_test() ->
    ...
    ExpectedKeys = [
        dh_ratchet_step, send_msg_number, recv_msg_number, 
        prev_recv_chain_length, skipped_keys_count,
        sending_chain_active, receiving_chain_active,
        created_at, last_updated, has_remote_dh
    ],
...
cleanup_expired_keys_test() ->
    ...
    OldTime = erlang:system_time(millisecond) - 90000000, % Very old
    NewTime = erlang:system_time(millisecond) - 1000,      % Recent
    ...
    StateWithSkippedKeys = AliceState#ratchet_state{skipped_keys = MockSkippedKeys},
    
    % Cleanup expired keys
    CleanedState = cryptic_double_ratchet:cleanup_expired_keys(StateWithSkippedKeys),
...
key_derivation_performance_test() ->
    ...
    io:format("~n1000 key derivations completed in ~p microseconds~n", [Duration]).
...
key_uniqueness_test() ->
    ...
    {Key1_1, Key1_2, Key1_3} = cryptic_double_ratchet:kdf_rk(RootKey1, DHOutput),
    {Key2_1, Key2_2, Key2_3} = cryptic_double_ratchet:kdf_rk(RootKey2, DHOutput),
...
full_initialization_flow_test() ->
    ...
    ?assertEqual(SharedSecret1, SharedSecret2), % Verify X3DH worked
    ...
    AliceState = cryptic_double_ratchet:init_sender(SharedSecret1, {AliceRatchetPub, AliceRatchetPriv}),
    BobState = cryptic_double_ratchet:init_receiver(SharedSecret2, {BobRatchetPub, BobRatchetPriv}, AliceRatchetPub),
    ...
    ?assertEqual(AliceRatchetPub, BobState#ratchet_state.dh_remote),
    
    % 4. Test that Alice can encrypt
    {ok, _Message, _NewAliceState} = cryptic_double_ratchet:encrypt_message(<<"Test">>, AliceState),
    
    % 5. Test that Bob initially cannot encrypt (receiving only)
    ?assertEqual({error, sending_chain_not_active}, 
